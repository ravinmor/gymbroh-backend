...
export class UserController {
	...
	async handle(request: Request, response: Response) {
		...
		const reqBody = request.body._parts ? request.body._parts : request.body;

		console.log(request.body);
		console.log('> ');

		let body: any = {};
		reqBody.map((item: any) => {
			body[item[0]] = item[1]
		})
		
		console.log(body);
		const result = await createUserService.store(body, request.files as Express.Multer.File[]);
